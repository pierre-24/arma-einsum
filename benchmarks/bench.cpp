...
BENCHMARK(BM_etrace)->Range(2 << 4, 2 << 8);
BENCHMARK(BM_atrace)->Range(2 << 4, 2 << 8);
...
static void BM_edaxpy(benchmark::State& state) {
  ...
  for (auto _ : state) {
    arma::Mat<BENCH_FLOAT> r = armaeinsum::einsum_mat_opt<BENCH_FLOAT>(armaeinsum::Greedy, "ik,k", A, b);
  }
}
...
static void BM_egemm(benchmark::State& state) {
  ...
  for (auto _ : state) {
    arma::Mat<BENCH_FLOAT> r = armaeinsum::einsum_mat_opt<BENCH_FLOAT>(armaeinsum::Greedy, "ik,kj", A, B);
  }
}
...
BENCHMARK(BM_egemm)->Range(2 << 4, 2 << 8);
BENCHMARK(BM_agemm)->Range(2 << 4, 2 << 8);
